...
// useEffect(() => {
//   api.get("/appointments/get-appointments-list/")
//     .then(response => {
//       setAppointments(response.data.data.appointments);  // assuming the server sends { appointments: [], stats: {...} }
//       setStats(response.data.data.stats);               // separate state for stats
//     })
//     .catch(error => {
...
// const handleTimeRangeChange = (e) => setTimeRange(e.target.value);  // Handle the time range change
...
//     const matchesQuery =
//       (item.patient_name?.toLowerCase().includes(lowerQuery) ?? false) ||
//       (item.appointment_id?.toString().toLowerCase().includes(lowerQuery) ??
//         false) ||
//       (item.phno?.toString().includes(lowerQuery) ?? false) ||
//       (item.doctor_name?.toString().includes(lowerQuery) ?? false) ||
//       (item.appointment_type?.toLowerCase().includes(lowerQuery) ?? false);

//     // Filter by time range (based on the timeRange state)
//     const matchesTimeRange =
//       timeRange === "all" ||
//       (timeRange === "week" &&
//         new Date(item.date) >= new Date() &&
//         new Date(item.date) <= new Date().setDate(new Date().getDate() + 7)) ||
//       (timeRange === "month" &&
...
//         </div>
//         <Button className={styles.buttonclick} onClick={() => setShowUserModal(true)}><CiCalendar className="mb-1"/> Schedule Appointments</Button>
//       </div>
...
//                   <input
//                     className={` ${styles.searchbar} ps-1 w-100`}
//                     type="search"
//                     value={query}
...
//                 />
//                 <select
//                   className={`${styles.selection} ms-1 w-25 me-2`}
//                   value={filter}
...
//     <Card key={item.id} className={styles.appointmentCard + " shadow-lg"} onClick={() => setShowModal1(true)}>
...
//           <AppointmentDetailsModal show={showModal1} handleClose={() => setShowModal1(false)}/>
//           </Card>  
//         </Col>
//       </Row>
//     </div>
