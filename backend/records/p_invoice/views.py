...
import traceback
from p_invoice.models import PharmacyInvoice, PharmacyInvoiceItem, Notification
...
class CreatePharmacyInvoiceAPIView(APIView):
    def post(self, request, *args, **kwargs):
        try:
            with transaction.atomic():
                ...
                # Call notification handler AFTER invoice and items are saved
                self.create_invoice_notification(invoice)
    ...
    def create_invoice_notification(self, invoice):
        ...
        if item_count == 0 or invoice.paid_amount <= 0:
            return

        formatted_amount = f"{invoice.paid_amount:.2f}"

        # Prevent duplicate sale notification
        ...
        if not existing_notification:
            Notification.objects.create(
                title=f"Pharmacy Sale {invoice.Bill_No}",
                message=(
                    f"Pharmacy sale {invoice.Bill_No} for Rs {formatted_amount} has been completed "
                    f"for {invoice.patient_name}. Items: {item_count}. Payment: {invoice.typeof_transaction}."
                )
            )

        # Mark invoice as notified
        invoice.notification_created = True
        invoice.save(update_fields=["notification_created"])
